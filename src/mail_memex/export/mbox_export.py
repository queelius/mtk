...
class MboxExporter(Exporter):
    ...
    Includes X-Mail-Memex-Tags header for tag preservation.
    """
    ...
    def _create_message(self, email_data: dict[str, Any]) -> EmailMessage:
        ...
        # Custom mail-memex headers
        tags = email_data.get("tags", [])
        if tags:
            msg["X-Mail-Memex-Tags"] = ", ".join(tags)

        thread_id = email_data.get("thread_id")
        if thread_id:
            msg["X-Mail-Memex-Thread-ID"] = thread_id
