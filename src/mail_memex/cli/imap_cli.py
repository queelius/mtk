"""IMAP CLI sub-app for mail-memex.
...
def _get_password(account: ImapAccountConfig) -> str:
    ...
    if not password:
        ...
        console.print("Run 'mail-memex imap auth' first, or enter password now:")
...
def imap_add(
    ...
    if json:
        ...
        console.print("\n[dim]Run 'mail-memex imap auth {name}' to store credentials[/dim]")
...
def imap_accounts(
    ...
    if not config.imap_accounts:
        ...
        console.print("[dim]Run 'mail-memex imap add' to add an account[/dim]")
        return
...
def imap_status(
    ...
    from mail_memex.cli.main import get_db
    from mail_memex.imap import ImapSync
    ...
    if json:
        ...
        if status["folders"]:
            ...
            console.print("  [yellow]No sync history (run 'mail-memex imap sync' first)[/yellow]")
